...
### 목업 모드
- 모든 API는 현재 목업 데이터 반환
- 실제 백엔드 연동은 코드 내 `TODO` 주석으로 표시
...
- 백엔드 연동: 시계열 + 마르코프 체인 예측값 수신 (TODO)

### 다음 단계
1. 백엔드 API 연동 브랜치 병합 후 `/api/preprocessing` 실제 구현 사용
2. 시계열 + 마르코프 체인으로 `/api/moods/current/generate` 실제 구현 완료
...
Firestore는 WearOS 디바이스와 ML 서버, Web 앱 간의 실시간 데이터 브리지 역할
...
- **Watch 앱 (WearOS)**: Firestore 연결 완료 (`google-services.json` 설정 완료)
- **Web 앱 (Next.js)**: Firestore 미연결 (현재 TODO 상태, V1 Mock 모드 사용 중)
- **ML Python 서버**: Firestore 연결 완료 (서비스 계정 키로 접근)

---

이 문서는 프론트엔드 개발 시 참고하는 API 명세
