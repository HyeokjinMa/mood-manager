...
Mood Manager는 생체 신호 분석, 오디오 이벤트 분류, 감정 예측, 무드 생성 등 다양한 AI 기술을 활용하여 개인화된 무드 스트림을 생성하는 시스템
...
### 기술적 한계
- 전통적인 통계 기반 방법론, 최신 딥러닝 기법 아님
- 통계 분석 및 패턴 인식 용도로 제한적 사용
...
#### 2.3 프롬프트 엔지니어링
- **다양성 강조**: 배경 요소와 음악의 다양성을 보장하기 위한 프롬프트 최적화
- **구조화된 출력**: JSON Schema를 통한 구조화된 응답 보장
- **컨텍스트 통합**: 사용자 선호도, 생체 신호, 외부 요인(날씨) 통합

### 기술적 특징
- **스트리밍 응답**: 대용량 세그먼트 생성 시 스트리밍으로 응답 시간 최적화
- **캐싱**: 동일 입력에 대한 중복 LLM 호출 방지
- **에러 처리**: LLM 호출 실패 시 폴백 무드 데이터 제공
...
### 3.1 생체 신호 수집
- **Health Services API**: 심박수, HRV, 호흡수, 스트레스 지수를 실시간으로 수집
- **주기적 수집**: 5분 간격으로 데이터를 수집하고 Firebase에 업로드
- **Foreground Service**: `PeriodicDataService.kt`에서 백그라운드로 지속 실행

### 3.2 오디오 이벤트 분류
- **AudioRecord API**: 2초 오디오 캡처
- **신호 처리**:
  - RMS/dBFS 계산으로 무음 구간 필터링
  - PCM 데이터를 WAV/Base64로 변환
- **Firebase 업로드**: `AudioEventService.kt`에서 ML 서버 처리를 위한 데이터 전송

### 기술적 특징
- **배터리 최적화**: 효율적인 데이터 수집 주기 설정
- **데이터 압축**: Base64 인코딩으로 네트워크 전송 최적화
- **에러 복구**: 네트워크 오류 시 재시도 메커니즘 사용
...
### 사용 모델
- **Wav2Vec2 기반**: 오디오 이벤트 분류 모델 (웃음, 한숨, 부정적 감정, 노이즈)
- **ONNX 최적화**: 모델 경량화 및 추론 속도 향상
- **양자화 (Quantization)**: 모델 크기 및 추론 시간 최적화

### 배포 전략
- **Docker 컨테이너화**: 일관된 실행 환경 보장
- **AWS Lambda**: 서버리스 아키텍처로 비용 최적화
- **비동기 처리**: Firestore의 `ml_processed` 플래그를 통한 작업 큐 관리

### 실제 적용
- **입력**: Firestore에 저장된 Base64 WAV 오디오 데이터
- **출력**: 분류된 이벤트 타입 및 타임스탬프
- **처리 흐름**: `ml_processed == 'pending'` 문서 조회 → 처리 → `'done'`으로 업데이트
...
- **생체 신호 정규화**: 심박수, HRV, 스트레스 지수 정규화
- **이벤트 집계**: 시간대별 오디오 이벤트 발생 빈도 계산
- **외부 데이터 통합**: 기상청 API를 통한 날씨 정보 수집
- **수면 점수 계산**: 수면 세션 기반 수면 품질 점수 산출
...
- **마르코프 체인**: 전통적인 통계 기반 방법론, 최신 딥러닝 기법 아님. 통계 분석 및 패턴 인식 용도로 제한적 사용
- **LLM 기반 생성**: 실제 서비스에서 무드 스트림의 핵심 생성 엔진으로 사용
- **ML 오디오 분류**: 실시간 오디오 이벤트 분류를 위한 경량화된 모델 사용
