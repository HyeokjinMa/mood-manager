...
Mood Manager는 생체 신호 분석, 오디오 이벤트 분류, 감정 예측, 무드 생성 등 다양한 AI 기술을 활용하여 개인화된 무드 스트림을 생성합니다.
...
### 기술적 한계
- 전통적인 통계 기반 방법론으로, 최신 딥러닝 기법은 아님
- 통계 분석 및 패턴 인식 용도로 제한적으로 사용
...
#### 2.3 프롬프트 엔지니어링
- **다양성 강조**: 아이콘과 음악의 다양성을 보장하기 위한 프롬프트 최적화
...
### 3.1 생체 신호 수집
- **Health Services API**: 심박수, HRV, 호흡수, 스트레스 지수 실시간 수집
- **주기적 수집**: 5분 간격으로 데이터 수집 및 Firebase 업로드
- **Foreground Service**: `PeriodicDataService.kt`에서 백그라운드 지속 실행
...
### 사용 모델
- **Wav2Vec2 기반**: 오디오 이벤트 분류 (웃음, 한숨, 부정적 감정, 노이즈)
...
- **처리 흐름**: `ml_processed == 'pending'` 문서를 조회 → 처리 → `'done'`으로 업데이트
...
- **마르코프 체인**: 전통적인 통계 기반 방법론으로, 최신 딥러닝 기법은 아님. 통계 분석 및 패턴 인식 용도로 제한적으로 사용됨.
- **LLM 기반 생성**: 실제 서비스에서 무드 스트림의 핵심 생성 엔진으로 사용됨.
