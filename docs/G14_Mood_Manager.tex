...
\textit{a) Current State of Smart Home Systems}
...
\textit{b) Current Trends in the Home Fragrance Market}
...
\textit{c) Need for Integrated Multi-Sensory Control}
...
Alexa Routines software enables users to automate tasks such as turning on lights or playing music based on time, sensor data, or voice triggers. It can execute multiple actions under a predefined "mood" routine (e.g., "Good Morning" or "Relax"). However, its logic is pre-programmed rather than contextual; it does not automatically adapt to environmental changes such as weather or user emotional state.
...
\textit{a) AI-Powered Context-Aware Analysis}
...
\textit{c) Manual Adjustment and Override}
...
\textit{a) Integration with Smart Lighting Devices}
...
\textit{b) Brightness and Color Temperature Adjustment}
...
\textit{c) Dynamic Lighting Effects}
...
\textit{c) Safety and Refill Monitoring}
...
\textit{a) Music and Soundscape Selection}
...
\textit{b) Volume and Playback Scheduling}
...
\textit{a) Conversational Feedback Window}
...
\textit{c) Quick Feedback Buttons}
...
\textit{a) Amazon Web Services(AWS)}
...
\textbf{1. Email Verification Request}
...
\subsubsection{Mood Set Display}
...
\subsubsection{Current Segment Replacement}
...
\subsubsection{Saved Mood Deletion}
...
\textbf{Directory} & \textbf{File Name} \\
\hline
mood-manager-ML/mood-manager-ML/apps/ml\_classification & predict.py, sigh\_laugh\_neg\_cls.py, convert\_onnx.py, dataset\_download.py, ml\_settings.py, fix\_config.py, requirements\_docker.txt, Dockerfile, .dockerignore \\
\hline
mood-manager-ML/mood-manager-ML/apps/ml\_classification/onnx\_model & model.onnx, model\_quantized.onnx, config.json, ort\_config.json, preprocessor\_config.json \\
\hline
mood-manager-ML/mood-manager-ML/apps/ml\_classification/saved\_model & model.safetensors, config.json, preprocessor\_config.json \\
\hline
markov & build\_yesterday\_many.py, realtime\_inference\_many.py \\
\hline
\end{tabular}
\end{table}
...
- sigh\_laugh\_neg\_cls.py : Implements the main classifier for sigh/laugh/negative categories.

- convert\_onnx.py : Converts trained PyTorch/TensorFlow models into ONNX format.

- dataset\_download.py : Downloads and organizes external datasets for emotion classification.

- ml\_settings.py : Configuration file defining model hyperparameters and thresholds.

- fix\_config.py : Applies patches or adjustments to existing model configuration files.

- requirements\_docker.txt : Lists dependencies for running ML service in Docker or isolated environments.
...
- onnx\_model/model.onnx : Base ONNX model for audio emotion classification.

- onnx\_model/model\_quantized.onnx : Optimized quantized ONNX model for runtime inference.

- onnx\_model/config.json : Model configuration parameters.

- onnx\_model/ort\_config.json : ONNX Runtime-specific configuration.

- onnx\_model/preprocessor\_config.json : Rules for audio preprocessing prior to model inference.

- saved\_model/model.safetensors : Model weights in SafeTensors format for secure serialization.

- saved\_model/config.json : Full model configuration for reproducibility.

- saved\_model/preprocessor\_config.json : Preprocessor settings for locally saved model.

- build\_yesterday\_many.py : Builds Markov-based mood prediction models from yesterday's raw data using DTW clustering.

- realtime\_inference\_many.py : Flask server for real-time mood state prediction using pre-trained Markov models.
...
\section{AI Specifications}

\subsection{Overview}

\subsubsection{System Role}
...
\subsubsection{Output Integration}
...
\subsection{DTW-Markov}

\subsubsection{DTW Similarity}
...
\subsubsection{Segment Construction}
...
\subsubsection{Naming Generation}
...
\subsubsection{Stylistic Coherence}
...
\subsubsection{Controlled Scope}
...
\subsubsection{DTW \& K-menas Clustering}

Our project explored several algorithmic methods to capture user state patterns, including DTW-based distance analysis and k-means clustering for grouping similar biometric patterns. These techniques were particularly useful for identifying trends in heart-rate–derived features and segmenting daily behavioral rhythms.

\subsubsection{Discrete Markov}
...
\bibitem{kotlin} "Kotlin Logo," \textit{TechIcons}. https://techicons.dev/icons/kotlin

\bibitem{typescript} "TypeScript Logo," \textit{TechIcons}. https://techicons.dev/icons/typescript

\bibitem{python} "Python Logo," \textit{TechIcons}. https://techicons.dev/icons/python

\bibitem{nextjs} "Next.js Logo," \textit{TechIcons}. https://techicons.dev/icons/nextjs

\bibitem{fastapi} "FastAPI Logo," \textit{TechIcons}. https://techicons.dev/icons/fastapi

\bibitem{tailwind} "TailwindCSS Logo," \textit{TechIcons}. https://techicons.dev/icons/tailwindcss

\bibitem{jetpack} "Android Jetpack Architecture Diagram," \textit{Android Developers Blog}, May 2019. https://android-developers.googleblog.com/2019/05/whatsnew-with-android-jetpack.html

\bibitem{aws} "Amazon Web Services Logo," \textit{TechIcons}. https://techicons.dev/icons/amazonwebservices

\bibitem{firebase} "Firebase Logo," \textit{TechIcons}. https://techicons.dev/icons/firebase

\bibitem{androidstudio} "Android Studio Logo," \textit{TechIcons}. https://techicons.dev/icons/androidstudio

\bibitem{vscode} "Visual Studio Code Logo," \textit{TechIcons}. https://techicons.dev/icons/vscode

\bibitem{github} "GitHub Logo," \textit{TechIcons}. https://techicons.dev/icons/github

\bibitem{figma} "Figma Logo," \textit{TechIcons}. https://techicons.dev/icons/figma

\bibitem{notion} "Notion Logo," \textit{Wikimedia Commons}. https://commons.wikimedia.org/wiki/File:Notion-logo.svg

\bibitem{postgresql} "PostgreSQL Logo," \textit{TechIcons}. https://techicons.dev/icons/postgresql

\bibitem{docker} "Docker Logo," \textit{TechIcons}. https://techicons.dev/icons/docker

\bibitem{chatgpt} "ChatGPT Logo," \textit{Wikimedia Commons}. https://upload.wikimedia.org/wikipedia/commons/e/ef/ChatGPT-Logo.svg

\bibitem{gemini} "Google Gemini Logo," \textit{Wikimedia Commons}. https://upload.wikimedia.org/wikipedia/commons/8/8f/Google-gemini-icon.svg
